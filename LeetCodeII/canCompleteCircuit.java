...
class canCompleteCircuit {
    public static int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        for(int start = 0; start < length; start++) {
            int step = 0, bal = 0, index = 0;
            while(step < length){
                index = (start + step) % length;
                bal = bal + gas[index] - cost[index];
//                System.out.println("start:" + start + " index:" + index + " bal:" + bal + " step:" + step) ;
                if ((step == length -1) && bal >= 0 ) {
                    return start;
                } else if (bal <=0) {
                    break;
                } else {
    ...
    public static void main(String[] args) {
        int[] gas = {1,2,3,4,5};
        int[] cost = {3,4,5,1,2};
        canCompleteCircuit.canCompleteCircuit(gas, cost);
    }
}
